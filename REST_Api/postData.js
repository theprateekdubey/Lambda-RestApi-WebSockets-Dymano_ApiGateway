...
function authAttributes(params, callback, requestBody) {
  if (requestBody !== "{}") {
    if (!params.Item.email_id) {
      // callback(null,{ body: JSON.stringify("The email id cannot contain an empty value")});
      // return false;
    ...
    if (!params.Item.message) {
      // callback(null,{ body: JSON.stringify("The message cannot contain an empty value")});
      // return false;
      return callback(null, {
        body: JSON.stringify("The message cannot contain an empty value"),
      });
    }
  }
